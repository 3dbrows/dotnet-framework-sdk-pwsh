...
RUN Start-Process -Wait -FilePath vs_buildtools.exe -ArgumentList "--quiet --wait --norestart --nocache modify --installPath `"%ProgramFiles(x86)%\Microsoft Visual Studio\2022\BuildTools`" --add Microsoft.VisualStudio.Workload.ManagedDesktopBuildTools --includeOptional"
